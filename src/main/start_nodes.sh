...
for ID in $(seq 1 $NUM_NODES) ; do
  ...
  if [ $ID -eq 1 ]; then
    ...
    sshpass -p ${DSV_PASS} ssh dsv@${NODE_IP[$ID]} -- "tmux send -t NODE_${ID} 'cd ${SEMWORK_HOMEDIR}/NODE_${ID}/ && java -cp ${FAT_JAR} cz.cvut.fel.dsva.Node ${ID} ${NODE_IP[$ID]} ${NODE_IP[1]} ${NODE_PORT[1]}' ENTER"
  fi
done
